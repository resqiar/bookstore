...
async function initBook() {
  const bookURL = document.getElementById("book_img");
  const bookTitle = document.getElementById("book_title");
  const bookDesc = document.getElementById("book_desc");
  ...
  try {
    ...
    if (result.status === 200) {
      ...
      bookURL.value = data.imageURL;
      bookTitle.value = data.title;
      bookDesc.value = data.description;
      bookDate.value = data.releaseDate;
...
async function editBook() {
  ...
  try {
    const req = await window.fetch(`/api/adm/book/edit/${id}`, {
      method: "POST",
      headers: {
...
function renderToast(title) {
  toastElem.innerHTML = `
      ...
      <div class="ml-3 text-sm font-normal">${title} Updated.</div>
      <button type="button"
